...
class PropertyVC: UIViewController, ExpandableViewDelegate {
    ...
    /**
     When the view appears, disable editing
     */
    ...
    /**
    Set up all UI interactions, delegates, targets
     */
...
//        disableEditables(false)
    ...
    /**
     Initialize property data of the current instance
     - Parameters:
        - model: property model assigned to the model in the local instance
        - read: read boolean assigned to local instance
     */
    ...
    /**
     Configure gesture recognizer such that it can be recognized alongside others (don't be "blocked")
     */
    ...
    /**
     Page back and forth on the scroll view according using the gesture recognizers
     - Parameter sender: gesture recognizer detecting the swipe
...
//        }
    ...
    /**
     Disable/enable the editing of fields
     - Parameter bool: new editing state
     */
...
//        }
    ...
    /**
     Update views knowing most recent data and state
     */
    func updateViews() {
...
//                self.scrollview
     ...
     /**
     Get the main VC from the singleton
     */
    class func getController() -> PropertyVC {
        return AppStoryboard.Main.shared.instantiateViewController(withIdentifier: PropertyVC.storyboard_id) as! PropertyVC
    }
...
//                                                self.disableEditables(false)
    ...
    /**
     Allow the user to edit the property type via a drop-dowm menu
     - Note: called via selector
     */
...
extension PropertyVC : GMSAutocompleteViewControllerDelegate {
    /**
...
extension PropertyVC : UIScrollViewDelegate {
    /**
...
extension PropertyVC {
    ...
    @objc func updateProfitLabels() {
        ...
        let exp = self.selected_model.calcAnnualExpenses()
        let rent = self.selected_model.calcAnnualRent()
        let NOIexp = self.selected_model.calcAnnualNOIExpenses()
        
        let monthly = (rent/12) - (exp/12)
        profit_month_text_field.value = monthly
        
        let annually = rent - exp
        profit_annual_text_field.value = annually
        
//        all_units_month_income_field.value = monthly
//        all_units_annual_income_field.value = annually
        
        let price = purchase_amt_text_field.value
        let tempNOI = annual_income - NOIexpenses
        
        all_units_annual_net_operating_income_label.value = NOIexp
        all_units_monthly_net_operating_income_label.value = NOIexp/12
...
//        cap_rate_text_field.value = cap
        ...
        //5958
        // let total = (sum of expenses)
        // total_text_field.value
...
extension PropertyVC : UICollectionViewDelegate, UICollectionViewDataSource {
    /**
