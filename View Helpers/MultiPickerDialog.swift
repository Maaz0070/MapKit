...
class MultiPickerdialog: UIView, UIPickerViewDelegate, UIPickerViewDataSource {
    func numberOfComponents(in pickerView: UIPickerView) -> Int {
        return 2
    }
    

    typealias PickerCallback = (_ value: [String],_ index: [Int]) -> Void
    ...
    func show(title: String, doneButtonTitle: String = "Done", cancelButtonTitle: String = "Cancel", options: [String], selected: [Int] = [0, 0], callback: @escaping PickerCallback) {
        ...
        self.picker.selectRow(selected.first!, inComponent: 0, animated: false)
        self.picker.selectRow(selected.last!, inComponent: 1, animated: false)
//        if selected != nil {
...
//        self.picker.setValue(UIColor(rgb: 0x333333), forKeyPath: "textColor")
    ...
    private func addButtonsToView(container: UIView) {
        ...
        self.cancelButton.setTitleColor(.darkText, for: UIControl.State.normal)
        self.cancelButton.setTitleColor(.darkGray, for: UIControl.State.highlighted)
        ...
        self.doneButton.setTitleColor(.darkText, for: UIControl.State.normal)
        self.doneButton.setTitleColor(.darkGray, for: UIControl.State.highlighted)
    ...
    @objc func buttonTapped(sender: UIButton!) {
        if sender.tag == kPickerDialogDoneButtonTag {
            ...
            let anotherIndex = self.picker.selectedRow(inComponent: 1)
            if self.pickerData.count != 0 {
                let selectedValue = self.pickerData[selectedIndex]
                let anotherValue = self.pickerData[anotherIndex]
                self.callback?([selectedValue, anotherValue], [selectedIndex, anotherIndex])
            }
        }
