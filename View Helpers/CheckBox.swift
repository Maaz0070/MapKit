...
open class CheckBox: UIControl {
    ...
    @IBInspectable
    var uncheckmarkColor: UIColor = #colorLiteral(red: 0.2549019754, green: 0.2745098174, blue: 0.3019607961, alpha: 1)
    ...
    @IBInspectable var shapeStyle:Int {
        get {
            return self.style.rawValue
        }
        set( shapeIndex) {
            self.style = Style(rawValue: shapeIndex) ?? .tick
        }
    }
    ...
    var radius: CGFloat = 2 {
        didSet {
            self.borderStyle = .roundedSquare(radius: radius)
            self.setNeedsDisplay()
        }
    }
    ...
    open override func draw(_ rect: CGRect) {
        ...
        if isChecked {
            switch self.style {
            case .square:
                self.drawInnerSquare(frame: newRect, color: checkmarkColor)
            case .circle:
                self.drawCircle(frame: newRect, color: checkmarkColor)
            case .cross:
                self.drawCross(frame: newRect)
            case .tick:
                self.drawCheckMark(frame: newRect, color: checkmarkColor)
            }
        } else {
            switch self.style {
            case .square:
                self.drawInnerSquare(frame: newRect, color: uncheckmarkColor)
            case .circle:
                self.drawCircle(frame: newRect, color: uncheckmarkColor)
            case .cross:
                self.drawCross(frame: newRect)
            case .tick:
                self.drawCheckMark(frame: newRect, color: uncheckmarkColor)
            }
        }
    }
    ...
    func drawCheckMark(frame: CGRect, color: UIColor) {
    ...
    func drawCircle(frame: CGRect, color: UIColor) {
    ...
    func drawInnerSquare(frame: CGRect, color: UIColor) {
        //// General Declarations
        // This non-generic function dramatically improves compilation times of complex expressions.
        func fastFloor(_ x: CGFloat) -> CGFloat { return floor(x) }
...
//        let padding = self.bounds.width * 0.3
        ...
        let rectanglePath = UIBezierPath.init(roundedRect: innerRect, cornerRadius: innerRect.width / 2)
